...
void invokeMethod(method_req_t *methodReq, res_struct *resObj)
{
    ...
    if (!rbus_handle)
    {
        ...
        resObj->u.methodRes->message = strdup("RBUS initialization failed");
        return;
    }
	...
	if ((outVal = rbusObject_GetValue(outParams, "statusCode")) != NULL)
    {
        ...
        //  WDMP will convert respose status codes to HTTP status codes, so if component sent HTTP status codes, map them to WDMP status codes
        if(status_code == 200)
            status_code = WDMP_SUCCESS;
        else if(status_code > 200)
            status_code = WDMP_FAILURE;
    }

	if (!return_message)
		return_message = (rc == RBUS_ERROR_SUCCESS) ? "Success" : rbusError_ToString(rc);
	if(status_code == -1)
		status_code = (rc == RBUS_ERROR_SUCCESS) ? WDMP_SUCCESS : WDMP_FAILURE; //If no status code was returned by the method or rbus failure, Use WDMP_FAILURE as a generic failure code

    resObj->u.methodRes->statusCode = status_code;
    resObj->u.methodRes->message = strdup(return_message);
}